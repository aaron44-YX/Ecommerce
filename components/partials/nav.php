<header>
        <nav class="flex items-center justify-between p-4 bg-white">
            <h1 class="text-green-600 text-2xl font-bold">Pearlas</h1>
            <div class="flex items-center space-x-2">
                <input type="search" placeholder="Search Pearlas" class="w-96 p-2 rounded bg-green-100 focus:outline-green-500">
                <ion-icon name="search" class="text-green-600 cursor-pointer"></ion-icon>
            </div>
            <div class="flex items-center space-x-4">
                <ion-icon name="cart" class="w-6 h-6 text-green-600 cursor-pointer"></ion-icon>
                <button id="user_login" onclick="redirectToLogin()" class="cursor-pointer px-4 py-2 rounded-full bg-green-600 text-white font-bold hover:bg-green-500">Login</button>
            </div>
        </nav>
    </header>